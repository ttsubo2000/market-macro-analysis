...
| インフレギャップ（コアCPI − 物価安定目標 2%） | 物価 | ✅ チャートあり | コアCPI から算出 | テイラールール定義 |
| 実質金利（政策金利 − BEI） | 金融 | ✅ 統合チャートに表示 | 政策金利・BEIから算出 | BEI 蓄積期間のみ描画 |
...
- **折れ線の分断**: データが5日を超えて欠測している区間を表します。取得元は常に最新10営業日分しか掲載していないため、`fetch_data` の実行間隔があくと欠測が生じます。分断された区間は「データが無い」という意味であり、その間の推移を示すものではありません

### インフレギャップチャート（`report/png/inflation_gap_chart.png`）

**インフレギャップ**（コアCPI − 物価安定目標 2%）の時系列棒グラフです。テイラールールにおける定義「実際のインフレ率 − 目標インフレ率」に基づきます。

- **プラス（赤）**: 物価上昇が目標（2%）を上回っている → 追加利上げ圧力
- **マイナス（青）**: 物価上昇が目標（2%）に届いていない → 緩和継続の余地あり

コアCPI データのみで計算するため、2016年〜現在の長期時系列が描画されます。

### 統合チャート（`report/png/combined_chart.png`）

「見取り図」の時計回り順（経済→物価→金融）に5指標を縦に並べた PNG を生成します。
...
| ② | コアCPI・物価上昇率 | 物価 |
| ③ | BEI・予想インフレ率 | 物価 |
| ④ | インフレギャップ（コアCPI − 2%目標） | 物価 |
| ⑤ | 政策金利・実質金利・自然利子率 | 金融 |

### マクロ経済考察レポート（`docs/report/macro_analysis_YYYYMMDD.md`）

`make_report all` または `make_report financial` 実行時に生成される Markdown 形式の考察レポートです。

**レポート構成**:

| セクション | 内容 |
|-----------|------|
| 1. 国内経済・物価指標の現状 | コアCPI・政策金利・需給ギャップ・BEI・実質金利の最新値テーブル |
| 2-1. 為替（USDJPY） | 直近30日間の為替動向と物価・金融政策への含意 |
| 2-2. 株式市場（日経225） | 直近30日間の株価動向とリスク環境の評価 |
| 3. 総合考察 | 上記指標を組み合わせた金融政策への含意コメント |

為替・株価データは [yfinance](https://pypi.org/project/yfinance/) 経由で Yahoo Finance から取得します。ネットワーク不通時は該当セクションのみスキップされ、国内指標の考察は継続して出力されます。

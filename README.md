...
| 予想インフレ率（企業物価見通しで代替） | 金融 | ⚠️ チャートあり（代替） | 日銀短観 co.zip | 本来は BEI（有料）を使うべき指標 |
...
#### なぜ「予想インフレ率」に企業物価見通しを使うのか

「予想インフレ率」の代表的な指標は **BEI（ブレーク・イーブン・インフレ率）** です。これは国債の名目利回りから物価連動国債の利回りを引いた値で、市場が将来の物価上昇を何%と見込んでいるかを示します。しかし BEI の取得には Bloomberg 等の有料データが必要です。

このツールでは代わりに **日銀短観の「企業物価見通し（1年後の消費者物価変化率・中央値）」** を使っています。これは日銀が四半期ごとに大企業・全産業計に「1年後の消費者物価は何%上がると思いますか？」と聞いた回答の中央値です。

| | BEI（本来） | 企業物価見通し（代替） |
|--|------------|-------------------|
| データ取得 | 有料（Bloomberg等） | 無料（日銀短観 co.zip） |
| 意味 | 市場参加者の期待インフレ | 大企業経営者の期待インフレ |
| 日銀の参照 | 重視 | 重視 |
| 過去データ | 物価連動国債の無料ソースなし | co.zip は最新四半期のみ収録 |

日銀が政策判断に直接参照する指標であり、無料で取得できる最良の代替として採用しています。

> **時系列が少ない理由：** 日銀短観の `co.zip` は最新四半期のみを収録しており、過去の時系列データは日銀 API でも提供されていません。`fetch_data` を四半期ごとに実行するたびに DB に1点ずつ蓄積される設計のため、運用開始直後はプロット数が少なくなります。

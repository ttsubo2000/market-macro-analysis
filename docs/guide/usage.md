...
financial_data : date, indicator, value, unit  ← 政策金利・企業物価見通し
economic_data  : date, indicator, value, unit  ← GDPギャップ・所定内給与前年比
```

**indicator 一覧**

| テーブル | indicator | 内容 | 頻度 |
|---------|-----------|------|------|
| price_data | core_cpi_yoy | コアCPI 前年比 | 月次 |
| financial_data | policy_rate | 政策金利（月平均） | 月次 |
| financial_data | expected_inflation_1y | 企業物価見通し 1年後 | 四半期 |
| economic_data | gdp_gap | 需給ギャップ | 四半期 |
| economic_data | scheduled_wage_yoy | 所定内給与前年比 | 月次 |
...
| `report/png/expected_inflation_chart.png` | 企業物価見通し（1年後）の四半期時系列グラフ |
| `report/png/combined_chart.png` | 4指標を縦に並べた統合チャート |
| `report/png/policy_rate_forecast.png` | 政策金利パス予測（現状評価＋4シナリオ・24ヶ月） |

### レポート（Markdown）

| ファイル | 内容 |
|---------|------|
| `report/summary.md` | 現状サマリー・利上げ確度スコア・シナリオ比較・政策金利パス予測を含む統合レポート |

---

## サマリーレポートの見方

`report/summary.md` は以下の4セクションで構成されます。
...
5条件の充足状況と利上げ確度スコア（0〜100%）を表示します。

| 条件 | 閾値 | 説明 |
|------|------|------|
| GDPギャップ | > 0% | 需要超過（需給がひっ迫） |
| コアCPI | > 1.5% | トレンドインフレが目標に近い |
| 予想インフレ率（1年後） | > 1.5% | 企業が物価上昇を見込んでいる |
| 賃金上昇率 | > 3.0% | 賃金と物価の好循環が継続 |
| 実質金利 < 自然利子率 | < 0.5% | 金融環境がまだ緩和的 |

### セクション 3: シナリオ比較

4シナリオ（ベースライン・楽観・リスク・スタグフレーション）の前提条件と各シナリオの利上げ確度スコアを比較表示します。

### セクション 4: 政策金利パス予測

現状評価スコアと各シナリオの利上げ確度から、次回利上げタイミングの目安（月数）を試算した表を表示します。利上げ幅は 0.25% を仮定。

---

## 取得データ一覧

| ブロック | 指標 | データソース | 更新頻度 | 備考 |
|---------|------|------------|---------|------|
| 物価 | コアCPI（生鮮食品除く・前年比） | 総務省 e-Stat API | 月次 | 直近120件（約10年） |
| 金融 | 無担保コールレート翌日物・月平均 | 日銀時系列統計DB CSV | 月次 | 1985年〜現在 |
| 金融 | 企業物価見通し（1年後・大企業全産業計・中央値） | 日銀短観 co.zip | 四半期 | fetch_data実行ごとに蓄積 |
| 経済 | 需給ギャップ（GDPギャップ） | 内閣府月例経済報告 xlsx | 四半期 | 1994年〜現在 |
| 経済 | 所定内給与前年比（就業形態計・産業計・5人以上） | 厚労省毎月勤労統計 e-Stat | 月次 | 動的コード解決（2フェーズAPI） |

> **日銀短観 co.zip の注意**: 最新四半期のみ収録。`fetch_data` を四半期ごとに実行することで時系列が蓄積されます。

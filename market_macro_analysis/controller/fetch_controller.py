...
from market_macro_analysis.service import estat_cpi_service, boj_rate_service, cao_gdp_service, mhlw_wage_service, boj_tankan_service
...
def fetch_economic_data(conn: sqlite3.Connection) -> None:
    ...
    app_id = load_estat_api_key(ESTAT_API_CONFIG_PATH)
    records = mhlw_wage_service.fetch_scheduled_wage_yoy(app_id)
    count = mhlw_wage_service.save_scheduled_wage(conn, records)
    print(f"[fetch_economic_data] 所定内給与前年比: {count} 件保存")
